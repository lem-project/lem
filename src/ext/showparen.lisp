...
(defun backward-matching-paren-default (point &optional (offset -1))
  (when (syntax-closed-paren-char-p (character-at point offset))
    (scan-lists (character-offset (copy-point point :temporary) (1+ offset))
                -1 0 t (window-view-point (current-window)))))

(defun show-paren-at-point (current-point &optional (offset -1))
  (let ((highlight-points '()))
    (or (when-let ((point (funcall (variable-value 'backward-matching-paren) current-point offset)))
