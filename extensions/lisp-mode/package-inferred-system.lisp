...
(defun insert-defpackage-and-in-package (point package-name)
  (with-open-stream (stream (make-buffer-output-stream point nil))
...
(defun replace-buffer-text-to-defpackage (buffer package-name)
  (let ((package-name (ensure-keyword package-name)))
    (with-buffer-read-only buffer nil
      (erase-buffer buffer)
      (insert-defpackage-and-in-package (buffer-point buffer) package-name))))
