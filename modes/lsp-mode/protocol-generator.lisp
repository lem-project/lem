...
(defun ensure-protocol-package ()
  (or (find-package :lem-lsp-mode/protocol)
      (make-package :lem-lsp-mode/protocol :use '())))

(defun translate-text (file-part &optional (stream *standard-output*))
  (let ((*package* (ensure-protocol-package)))
...
(defun generate (spec-file out-file)
  (with-open-file (out out-file
    ...
    (pprint `(defpackage :lem-lsp-mode/protocol)
            out)
    (pprint `(in-package :lem-lsp-mode/protocol)
            out)
