...
(defmacro with-eval ((&key (values (gensym) values-p)
                     ...
                     &body body)
  (alexandria:with-gensyms (input output io)
    `(with-open-stream (,input (make-minibuffer-input-stream))
       (with-open-stream (,output ,(if output-stream-p
                                       output-stream
                                       `(make-editor-output-stream #'write-string-to-repl)))
         (with-open-stream (,io (make-two-way-stream ,input ,output))
...
(defun self-interactive-eval (string)
  (with-eval (:values values)
    (message "=> ~{~S~^, ~}" values)))

(defun self-eval-print (string &optional print-right-margin)
  (declare (ignore print-right-margin))
  (with-eval (:values values :output-stream (make-buffer-output-stream (current-point)))
