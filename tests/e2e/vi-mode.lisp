  ...
  (test "enable hook"
    (test "initialize-vi-modeline"
      ...
      (ok (find-if (lambda (element)
                   ...
                   lem-internal::*modeline-status-list*)))
    (test "(change-state 'normal)"
      (ok (eq lem-vi-mode/core::*current-state* 'lem-vi-mode::command))

      (ok (eq (mode-keymap (get 'lem-vi-mode:vi-mode 'lem-internal::global-mode))
      ...
      (ok (string= (if (eq :dark (lem-internal::display-background-mode))
                       "white"
                       "dark")
