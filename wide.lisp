...
(defun %binary-search (vec val)
  (declare (optimize (speed 3) (safety 0) (debug 0)))
  (declare ((simple-vector #.(length *eastasian-full*)) vec)
           (fixnum val))
  (labels ((cmp (key val)
                (declare (fixnum key) (cons val))
                (cond ((<= (the fixnum (car val)) key (the fixnum (cadr val)))
                       0)
                      ((< key (the fixnum (car val)))
                       -1)
                      (t
                       1)))
           (rec (begin end)
...
(defun wide-char-p (c)
  (declare (character c))
  (%binary-search (load-time-value *eastasian-full*)
                  (char-code c)))
