...
(defun move-to-backward-open-paren (p)
  (let ((count 0))
    (loop while (and (not (and (syntax-open-paren-char-p (character-at p))
                               (= count 0)))
                     (/= (point-charpos p) 0))
          do (let ((ch (character-at p)))
               (when (syntax-open-paren-char-p ch)
                 (decf count))
               (when (syntax-closed-paren-char-p ch)
                 (incf count))
