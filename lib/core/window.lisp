...
(defun window-topleft-x (frame)
  (declare (ignore frame))
  0)
(defun window-max-width (frame)
  (- (display-width) (window-topleft-x frame)))
...
(defun change-display-size-hook ()
  ...
  (adjust-windows (window-topleft-x (current-frame))
                  (window-topleft-y (current-frame))
                  (+ (window-max-width (current-frame)) (window-topleft-x (current-frame)))
