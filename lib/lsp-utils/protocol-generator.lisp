...
(defparameter *specifications*
  '(("language-server-protocol/_specifications/specification-3-15.md"
     "protocol-3-15.lisp"
...
(defun emit-header-comment (stream spec-pathname)
  (format stream ";;; Code generated by ~A based on '~A'; DO NOT EDIT.~%"
          :lem-lsp-utils/protocol-generator
          (if (uiop:directory-pathname-p spec-pathname)
              (enough-namestring spec-pathname (asdf:system-source-directory :lem))
              (file-namestring spec-pathname))))

(defun emit-package-definition (stream package-name export-list)
  (pprint `(defpackage ,package-name
             (:import-from :lem-lsp-utils/json)
             (:import-from :lem-lsp-utils/type)
             (:export ,@(mapcar #'make-keyword export-list)))
          stream)
  (pprint `(in-package ,package-name) stream)
  (terpri stream)
...
(defun deploy ()
  (loop :for (spec-file output-file package-name) :in *specifications*
        :do (format t "~&generating ~S~%" output-file)
            (generate (asdf:system-relative-pathname :lem spec-file)
