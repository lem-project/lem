    ...
    (let ((window-panel (lem-panel-window-panel *lem-panel*)))
      (with-apply-in-pane-process-wait-single (window-panel)
        (when (window-panel-modified-p window-panel)
          ...
          (lem::adjust-windows (lem::window-topleft-x (lem:current-frame))
                               (lem::window-topleft-y (lem:current-frame))
                               (+ (lem::window-max-width (lem:current-frame)) (lem::window-topleft-x (lem:current-frame)))
                               (+ (lem::window-max-height (lem:current-frame)) (lem::window-topleft-y (lem:current-frame))))
